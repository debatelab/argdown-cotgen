...
This strategy reveals nodes using breadth-first traversal:
1. Start with all root nodes
2. Process nodes in queue order, expanding children when visited
3. Each step shows the children of one processed node
4. Add YAML and comments at the end
...
Note: Each step processes one node from the queue and shows its immediate children.
This follows true breadth-first search traversal order.
"""
...
class BreadthFirstStrategy(AbortionMixin, BaseArgumentMapStrategy):
    """
    ...
    def generate(self, parsed_structure: ArgdownStructure, abortion_rate: float = 0.0) -> List[CotStep]:
        ...
        if not isinstance(parsed_structure, ArgumentMapStructure):
            raise ValueError("BreadthFirstStrategy requires an ArgumentMapStructure")
        ...
        # Initialize breadth-first queue with root nodes
        ...
        while queue:
            ...
            if new_children:
                # Add new children to revealed set and queue
